...
export default function AITrainingPage() {
  const features = [
    {
      ...
      description: "Build tailored datasets for your AI needs with expert curation.",
      details: [
      ...
      stats: "10K+ datasets",
      icon2: <Database className="w-3 h-3" />
    },
    {
      icon: <Target className="w-8 h-8 text-[var(--lw-green)]" />,
      title: "Model Optimization",
      description: "Improve performance through high-quality, targeted training data.",
      details: [
        "Error analysis & data refinement",
        "Edge case identification",
        "Model-in-the-loop validation",
        "Continuous improvement cycles"
      ],
      stats: "+35% accuracy",
      icon2: <TrendingUp className="w-3 h-3" />
    },
    {
      icon: <Brain className="w-8 h-8 text-[var(--lw-green)]" />,
      title: "Domain Adaptation",
      description: "Specialized datasets for niche industries and use cases.",
      details: [
  ...
  const stats = [
    { number: "10K+", label: "datasets created", icon: <Database className="w-4 h-4" /> },
    { number: "99.7%", label: "label accuracy", icon: <Target className="w-4 h-4" /> },
    { number: "500+", label: "AI models trained", icon: <Brain className="w-4 h-4" /> },
    { number: "35%", label: "avg. improvement", icon: <TrendingUp className="w-4 h-4" /> }
  ]

  const datasetTypes = [
    { name: "Computer Vision", icon: <Eye className="w-4 h-4" />, description: "Images, video, object detection" },
    { name: "NLP", icon: <FileText className="w-4 h-4" />, description: "Text, sentiment, entity recognition" },
    { name: "Audio", icon: <Volume2 className="w-4 h-4" />, description: "Speech, sound events, transcription" },
    { name: "Multimodal", icon: <Layers className="w-4 h-4" />, description: "Combined data types" }
  ]

  const industries = [
    "Healthcare", "Automotive", "Finance", "Retail", "Robotics", "Agriculture"
  ]
      ...
      <main className="pt-24">
        ...
        <section className="relative overflow-hidden bg-gradient-to-b from-[var(--lw-green)]/[0.02] via-white to-white">
          ...
          <div className="relative max-w-7xl mx-auto px-6 py-16 text-center">
            ...
            <h1 className="text-6xl md:text-7xl font-bold mb-6 animate-slide-up">
              ...
              <br />
              <span className="text-[var(--lw-green)] animate-pulse-slow">Data</span>
            </h1>
            
            <p className="text-xl text-[var(--lw-dark)]/60 max-w-3xl mx-auto leading-relaxed animate-fade-in animation-delay-200">
              Custom datasets designed to train intelligent AI models. 
              Production-grade data for computer vision, NLP, and audio applications.
            </p>
              ...
              >
                <div className="flex justify-center mb-3">
                  <div className="w-12 h-12 bg-[var(--lw-green)]/[0.08] rounded-full flex items-center justify-center">
                    <div className="text-[var(--lw-green)]">{type.icon}</div>
                  </div>
                </div>
                <h3 className="font-semibold text-[var(--lw-dark)] mb-1">{type.name}</h3>
                <p className="text-xs text-[var(--lw-dark)]/40">{type.description}</p>
              </div>
            ))}
          </div>
        </section>
        ...
        <section className="max-w-7xl mx-auto px-6 py-12">
          <div className="text-center mb-12 animate-fade-in">
            <h2 className="text-3xl font-bold text-[var(--lw-dark)] mb-4">Training Data Solutions</h2>
            <p className="text-[var(--lw-dark)]/60 max-w-2xl mx-auto">
              Comprehensive datasets optimized for AI model performance
            </p>
          </div>
                ...
                <div className="relative h-24 bg-gradient-to-r from-[var(--lw-green)]/10 to-[var(--lw-green)]/5 overflow-hidden">
                  ...
                  <div className="absolute inset-0 flex items-center justify-around">
                    {[...Array(5)].map((_, i) => (
                      <div 
                        key={i}
                        className="w-1 bg-[var(--lw-green)]/30 rounded-full animate-pulse"
                        style={{ 
                          height: `${20 + i * 8}px`,
        ...
        {/* QUALITY METRICS - SIMPLE SECTION */}
          ...
          <div className="max-w-7xl mx-auto px-6">
            <div className="text-center mb-10 animate-fade-in">
              <div className="inline-flex items-center gap-2 bg-white/10 border border-white/20 px-4 py-2 rounded-full mb-4">
                ...
                <span className="text-sm uppercase tracking-wider text-white/80">Quality Guaranteed</span>
              </div>
              <h2 className="text-3xl font-bold mb-3">Ready for Production</h2>
              <p className="text-white/60 max-w-2xl mx-auto">
                Every dataset undergoes rigorous validation before delivery
              </p>
            </div>

            <div className="flex flex-wrap justify-center gap-8">
              {[
                { label: "Dataset Size", value: "10K+", icon: <Database className="w-5 h-5" /> },
                { label: "Avg. Accuracy", value: "99.7%", icon: <Target className="w-5 h-5" /> },
                { label: "Domains Covered", value: "20+", icon: <Globe className="w-5 h-5" /> }
              ].map((item, index) => (
                <div 
                  key={index}
                  className="flex items-center gap-4 bg-white/5 rounded-2xl px-6 py-4 border border-white/10 hover:shadow-[0_8px_20px_rgba(4,98,65,0.2)] transition-all duration-300 hover:-translate-y-1"
                >
                  <div className="text-[var(--lw-green)]">{item.icon}</div>
                  <div>
                    <div className="text-2xl font-bold text-[var(--lw-green)]">{item.value}</div>
        ...
        <section className="py-16 bg-white">
          <div className="max-w-7xl mx-auto px-6">
            <div className="text-center mb-10 animate-fade-in">
              ...
              <p className="text-[var(--lw-dark)]/60 max-w-2xl mx-auto">
                Specialized training data for diverse sectors
              </p>
            </div>
        ...
        <section className="max-w-7xl mx-auto px-6 py-16 text-center">
          <div className="bg-gradient-to-r from-[var(--lw-green)]/[0.05] to-[var(--lw-green)]/[0.02] rounded-3xl p-10 border border-[var(--lw-green)]/20 hover:shadow-[0_20px_40px_rgba(4,98,65,0.15)] transition-all duration-500">
            <h2 className="text-3xl font-bold text-[var(--lw-dark)] mb-3 animate-fade-in">Ready to Train Your AI?</h2>
            <p className="text-[var(--lw-dark)]/60 max-w-2xl mx-auto mb-6 animate-fade-in animation-delay-200">
              Get custom training data built for your specific use case
            </p>
            ...
            <p className="mt-4 text-sm text-[var(--lw-dark)]/40">
              <Brain className="inline mr-2 w-4 h-4 text-[var(--lw-green)] animate-pulse-slow" />
          ...
          <div className="max-w-7xl mx-auto px-6 py-8">
            <div className="flex flex-col md:flex-row justify-between items-center gap-4">
              ...
              <div className="text-sm text-[var(--lw-dark)]/40">
                training data for responsible AI
              </div>
              <div className="text-xs text-[var(--lw-dark)]/30">
                © 2025 · custom datasets · production ready
              </div>
            </div>
          </div>
        </footer>
